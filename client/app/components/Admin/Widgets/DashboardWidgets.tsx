...
const DashboardWidgets: FC<Props> = ({ open }) => {
  const [comparePercentenge, setcomparePercentenge] = useState<any>();
      ...
      if (data && ordersData) {
        ...
        if (
          ...
          setuserComparePercentage({
            ...
            precentChange: usersPercentChange,
          });

          setOrdersComparePercentage({
            currentMonth: ordersCurrentMonth,
            previousMonth: ordersPreviousMonth,
            precentChange: ordersPercentChange,
          });
        }
      }
    }
        ...
        <div className="pt-[80px] pr-8">
          <div className="w-full dark:bg-[#111C43] rounded-sm shadow">
            <div className="flex items-center p-5 justify-between">
              ...
              <div>
                <CircularProgressWithLabel value={ordersComparePercentage?.precentChange > 0 ? 100 : 0} open={open} />
                <h5 className="text-center pt-4">
                  {ordersComparePercentage?.precentChange > 0
                    ? "+" + ordersComparePercentage?.precentChange.toFixed(2)
                    : "-" +
                      ordersComparePercentage?.precentChange.toFixed(2)}{" "}
                  %
                </h5>
